...
init_action() {
  ...
  run_out "$ARTIFACT_DIR/00-preflight/whoami.txt" oc whoami
  {
    ...
    write_run_info_kv INCLUDE_SERIAL_DNS_TESTS "$INCLUDE_SERIAL_DNS_TESTS"
    write_run_info_kv DNS_TEST_EXCLUDE_REGEX "$DNS_TEST_EXCLUDE_REGEX"
    write_run_info_kv DNSPERF_QPS_STEPS "$DNSPERF_QPS_STEPS"
    write_run_info_kv DNSPERF_DURATION_SECONDS "$DNSPERF_DURATION_SECONDS"
    write_run_info_kv DNSPERF_CLIENTS "$DNSPERF_CLIENTS"
    write_run_info_kv DNSPERF_THREADS "$DNSPERF_THREADS"
    ...
    write_run_info_kv DNS_VALIDATION_REPORT_MODE "$DNS_VALIDATION_REPORT_MODE"
    write_run_info_kv DNS_VALIDATION_DEEP_DIAGNOSTICS "$DNS_VALIDATION_DEEP_DIAGNOSTICS"
